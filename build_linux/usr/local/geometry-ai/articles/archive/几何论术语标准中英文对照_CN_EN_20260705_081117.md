...
> **语言**：中文 / English  
> **更新规则**：见 §11 维护规则  
> **本次更新**：0.0.0《零与谱》第二版同步——新增本体前提/结构公理/编码公理三层结构、谱展开、谱权重 6:2:1、六项无量纲代价函数、严格凸性等约20个新术语；§1 公理体系重构以兼容 0.0.0 底层框架
...
| 谱权重 | Spectral Weight | $6:2:1$，由 $S_3$ 群代数 $\mathbb{C}[S_3]$ 的左理想维数给出，分别对应全对称层（$S_3$，维数 6）、循环层（$A_3$，维数 2）、平凡层（$\{e\}$，维数 1） | 0.0.0 §2 |
...
| 中文 | 英文 | 公式 / 陈述 | 出处 |
|---|---|---|---|
| 三分切丛定理 | Tripartite Tangent Bundle Theorem | $TM(a)=\mathcal M\oplus\mathcal C\oplus\mathcal I$，三个 $S^3$ 因子切丛的拉回，每个纤维同构于 $\mathbb R^3$。三扇区在数学地位上完全等价，$S_3$ 置换诱导切丛分解的自同构 | 0.0.0 §5.1 |
...
| 值域定理与严格凸性 | Range Theorem & Strict Convexity | 六项代价函数 $S$ 在约束定义域 $D_\theta=\{(\theta_1,\theta_2,\theta_3)\mid\theta_i>0,\;\theta_1+\theta_2+\theta_3=90^\circ\}$ 上严格凸，值域为 $[24,+\infty)$，唯一全局严格最小值点为 $\theta_1=\theta_2=\theta_3=30^\circ$，$S_{\min}=24$ | 0.0.0 §5.6 |
| 模空间一维性引理 | Moduli Space One-Dimensionality Lemma | 约束乘积球面 $M(a)$ 的连续模空间由单一尺度参数 $a\in(0,\infty)$ 参数化，同胚于 $(0,\infty)$ | 0.0.0 §5.6 |
