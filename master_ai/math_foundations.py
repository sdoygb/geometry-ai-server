...
# ==================== 唯一物理映射 ℰ（Layer 2） ====================
# 几何论的核心设定：ℰ 是单一物理映射，把几何量映射为物理可观测量。
# 这是唯一的外部物理假设。所有其他物理概念（Born规则、物理识别、归一化等）
# 都必须从 ℰ 推导出来，验证入库后成为 L3 绝对真理。

BRIDGE_HYPOTHESES: Dict[str, Dict[str, str]] = {
    "epsilon_mapping": {
        "name": "ℰ 单一物理映射",
        "description": (
            "ℰ: 几何量 → 物理可观测量。这是几何论唯一的外部物理假设。"
            "ℰ 做一件事且只做一次：将角度参数空间中的几何量映射为物理世界中的可观测量。"
            "Born规则、物理识别点、归一化约定等都应该是 ℰ 的推论，不是独立假设。"
        ),
        "usage": "作为几何论与物理世界的唯一接口。所有物理概念必须从ℰ推导。",
    },
}

# ==================== 待推导物理定理清单 ====================
# 这些概念之前被当作独立的"L2桥接假设"，但根据单一ℰ原则，
# 它们必须从 ℰ 推导出来、验证入库后才能作为 L3 使用。
# 当前都不在主库中 → 如果子AI的推导依赖它们，应返回依赖缺口：
# "请先从ℰ推导Born规则并提交验证"
...
PENDING_PHYSICS_DERIVATIONS: List[Dict[str, str]] = [
    {
        "name": "Born规则",
        "target": "P(i) = |⟨i|ψ⟩|² = |c_i|²",
        "description": "测量概率等于振幅模方。必须从ℰ推导：ℰ如何将几何振幅映射为概率？",
        "dependency": "ℰ → Born规则",
    },
    {
        "name": "物理识别点",
        "target": "几何约束截面 → 粒子种类",
        "description": "几何论中的约束截面对应物理粒子种类。必须从ℰ推导：ℰ如何将几何截面映射为粒子身份？",
        "dependency": "ℰ → 物理识别点",
    },
    {
        "name": "归一化约定",
        "target": "谱 → 耦合常数",
        "description": "几何谱归一化对应物理耦合常数。必须从ℰ推导：ℰ如何将几何谱映射为耦合常数？",
        "dependency": "ℰ → 归一化约定",
    },
    {
        "name": "量子-经典对应",
        "target": "ℏ→0 极限下量子→经典",
        "description": "Bohr对应原理。必须从ℰ推导或验证：ℰ在经典极限下如何退化？",
        "dependency": "ℰ → 量子-经典对应",
    },
]
