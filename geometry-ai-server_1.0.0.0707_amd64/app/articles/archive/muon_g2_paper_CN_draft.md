...
电磁耦合常数 $\alpha$ 在框架中由 $S$ 的稳态值 $\alpha = 1/S_e$ 非微扰地确定，其中 $S_e = 137.035999084$ 为电子构型对应的作用量值。反常磁矩 $a_\ell$ 则由约束流形上的辛几何结构通过 Hamilton 流的路径平均自然导出。
...
其中 $S_e = 137.035999084$ 是电子构型对应的作用量值。这一赋值并非拟合——$S_e$ 由作用量方程组的精确求解给出，输入量仅为三条公理中的常数 $K = 839.758793$ keV 和电子质量 $m_e = 510.998950$ keV（$m_e$ 用于标定 $K$，$K$ 一旦标定后即为普适常数）。
...
公理3将带电轻子的质量与其物质界角度直接关联。质量量子 $K = 839.758793$ keV 是普适常数。给定轻子质量 $m_\ell$，物质界角度唯一确定：
...
电子构型在框架中具有特殊地位。电子作用量 $S_e = 137.035999084$ 是框架的数值锁定锚点。电子构型 $P_e$ 处于约束流形的特殊区域，满足以下两个闭包条件：

**条件一（§1.4）**：电子与隐Hessian轻态构成最轻的闭合电磁耦合对，其作用量精确等于精细结构常数倒数。

**条件二（§2.2）**：在约束流形的辛结构中，电子构型对应 Hamilton 流的主通道端点。

电子构型的完整角度组态：
$$\boxed{P_e = (\theta_M^e \approx 26.726^\circ, \theta_C^e \approx 49.894^\circ, \theta_I^e \approx 13.380^\circ)} \tag{8}$$

该组态源自对电子约束方程的精确求解，详见附录A。
...
该机制的核心在于：电磁耦合的几何表示 $Q$ 在家族间的近似普适性。如果这一普适性是精确的（或破缺很小），则标准模型中通过 Yukawa 耦合参数化的"家族结构"在几何论中退化为角度空间中的几何轨道族——每个家族共享相同的 $\theta_C$–$\theta_I$ 耦合强度，仅在 $\theta_M$ 上有所区别。
...
领头阶贡献 $M_1$ 由约束流形在 $P_e$ 处的局部几何决定。计算 Hessian 矩阵在 $P_e$ 处的本征结构，利用鞍点展开得到：
...
**整数部分 137**：对应"裸基准点" $\tilde{P}_0$，即 $S$ 取严格整数 137 的约束流形上的构型。

**尾数部分 0.035999084**：对应从 $\tilde{P}_0$ 到真实电子构型 $P_e$ 的小位移。该位移由闭合条件（电子与隐Hessian轻态耦合形成最轻电磁对）驱动，在约束流形的辛结构中对应于 Hamilton 流端点。
...
约束流形 $\Sigma$ 上承载自然的辛结构。在局部坐标 $(\xi, \eta)$ 下，定义 Hamilton 函数为六项作用量 $S(\xi, \eta)$：
...
μ子的反常磁矩由五因子乘积给出（对应9号文章 §7.2-7.3）：
...
5. **$\mathcal{F}_{\text{norm}}$ — 归一化修正**：
   ...
   其中 $\mathcal{N}_\Sigma = \pi/\sqrt{8}$（见附录C）是约束截面整体归一化，$\mathcal{N}_e$ 是电子局域归一化。
...
1. **零自由参数**：本计算未引入任何拟合参数。三条公理中的常数 $K = 839.758793$ keV 由电子质量标定（一次性普适标定），此后对所有轻子通用。
...
几何论与标准模型预言之间的数值接近程度是引人注目的（偏差 < 0.05%），但两者的理论基础完全不同：
...
**2. 色散关系与格点 QCD**：标准模型 $a_\mu$ 的理论不确定度主要由强子贡献（HVP + HLbL）主导，目前存在色散关系与格点之间的张力 [3]。几何方法绕过了强子贡献的直接计算——在约束流形上，强相互作用的几何来源与电磁相互作用共享相同的 $Q$ 不变量结构（见9号文章 §8），但这一连接的严格推导尚未完成。
...
| 地位 | 要素 |
|:---|:---|
| **定理** | 三公理体系；完备性约束；六项作用量形式；质量映射公理；$Q$ 不变量定义；电子 Schwinger 项还原；Hessian 展开的领头项系数 |
...
### A.1 基础常数
$$\begin{aligned}
K &= 839.758793 \text{ keV} \quad (\text{质量量子，定理级}) \\
S_e &= 137.035999084 \quad (\text{电子作用量，定理级}) \\
S_\mu^{\text{eff}} &= 137.0263(3) \quad (\text{μ子有效作用量，构造性计算}) \\
\lambda_1^{\text{eff}} &= 391.05 \text{ rad}^{-2} \quad (\text{约束截面有效刚度}) \\
\lambda_2^{\text{eff}} &= 59324.3 \text{ rad}^{-2} \quad (\text{辅助刚度参数})
\end{aligned}$$
...
约束流形 $\Sigma$ 单点紧致化后同胚于 $S^2$（$\bar{\Sigma} \cong S^2$），欧拉示性数 $\chi(S^2) = 2$。由 Gauss-Bonnet 定理：
$$\int_{S^2} K_G \, dA = 2\pi\chi = 4\pi$$

其中 $K_G$ 是 $S^2$ 上的高斯曲率。

Clifford 代数 Cl(9) → Cl(3,1) 约化中，内部 5 个生成元（$9 - 4 = 5$）贡献旋量归一化因子：
$$\mathcal{N}_{\text{spinor}} = 2^{(d_{\text{int}})/2} = 2^{5/2} = 4\sqrt{2}$$

约束截面的全局归一化因子为上述两个几何量的组合：
...
该因子在约束截面上的标量零模质量归一化中以 $\mathcal{N}_\Sigma$ 形式进入。推导的完整步骤见4号文章定理6.2。

**诚实标注**：除数关系 $\propto 1/\mathcal{N}_{\text{spinor}}$ 的严格论证依赖 $S^2$ 上的超对称痕量公式（玻色子-费米子行列式之比）。Clifford 因子 $4\sqrt{2}$ 本身来自 Cl(9) 结构（定理级），但以除数形式进入的证明属"可封闭的构造性环节"。

---

## 附录D：与完整九元素体系的关系

本文聚焦于反常磁矩的计算通道。在本框架的完整体系中（见内部参考 [G1–G12]），带电轻子的性质由九个相互耦合的元素共同决定：
...
| 约束截面 $\Sigma$ | §II 约束流形 |
| 六项作用量 $S$ | §II.B, §V |
| 单一核心映射层 | §IV $Q$ 不变量与电磁耦合 |
| $Q$ 不变量 | §IV 核心映射机制 |
| 梯度场 $\nabla S, \nabla Q$ | §V 电磁矩的几何来源 |
| Hessian 度量 | §V.B 领头阶展开, 附录B |
| Hamilton 流 | §VII 路径平均 |
| 九素互扼 | 各元素间的交叉锁定（本文隐式使用） |

九元素之间的互扼关系保证了框架整体的自洽性：改变任一元素必然牵动其余元素。这使得本文的 $a_\mu$ 预言不仅是一个孤立数值——它与框架的所有其他预言（希格斯质量、中微子质量、CKM 混合、强 CP 等）共享同一组基本常数。
...
[2] B. Abi et al. (Muon g-2 Collaboration), Phys. Rev. Lett. **126**, 141801 (2021); D. P. Aguillard et al. (Muon g-2 Collaboration), Phys. Rev. Lett. **131**, 161802 (2023).

[3] T. Aoyama et al., Phys. Rept. **887**, 1 (2020); 关于 HVP 张力，参见 Sz. Borsanyi et al., Nature **593**, 51 (2021) (格点 QCD) 与 M. Davier et al., Eur. Phys. J. C **80**, 241 (2020) (色散关系) 的比较。

[4] J. Schwinger, Phys. Rev. **73**, 416 (1948).

[5] D. Hanneke, S. Fogwell, and G. Gabrielse, Phys. Rev. Lett. **100**, 120801 (2008); X. Fan et al., Phys. Rev. Lett. **130**, 071801 (2023).

[6] Muon g-2 世界平均:物理评论D综述（2024更新），见 Particle Data Group, Prog. Theor. Exp. Phys. **2024**, 083C01 (2024).

[7] T. Aoyama, M. Hayakawa, T. Kinoshita, and M. Nio, Phys. Rev. Lett. **109**, 111807 (2012); **109**, 111808 (2012); S. Laporta, Phys. Lett. B **772**, 232 (2017).

---

## 内部参考 [G系列]

[G1] 欧阳国彬,「0.0.3 几何论公理体系」, 内部文章 260622.6.

[G2] 欧阳国彬,「0.1 约束截面与Hessian几何」, 内部文章 260622.6.

[G3] 欧阳国彬,「0.3.1 质量量子K的锁定」, 内部文章 260622.6.

[G4] 欧阳国彬,「4 希格斯呼吸模式」, 内部文章 260622.6.

[G5] 欧阳国彬,「7 中微子集群效应」, 内部文章 260622.6.

[G6] 欧阳国彬,「9 反常磁矩的完整推导」, 内部文章 260622.6.
