"""
geometry_ai_server_v9_living.py
几何论AI调度中间层 — 活体信息场版
版本：v9.0

改造目标（基于 v8 向量数据库+学习闭环版）：
1. eta 改为纯内存实时计算（LivingInfoField 类），不再从数据库读写
2. 建立自指反馈环：KIMI 回复的几何论术语密度反馈回 eta 动力学
3. 新增 extract_key_propositions / compute_geo_density 函数
4. eta 初始值由输入文本的软模共振公式计算（而非固定30度）
5. 保留所有现有功能：VectorKnowledgeBase（ChromaDB）、MySQL（对话记录）、文件解析、质量门控等
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('geometry_ai_v9.log', encoding='utf-8')
    ]
)
...
几何论术语密度: {metrics.get("geo_density", 0):.4f}
...
def _finalize_turn(
    ...
    """
    v9 改造：
    - 从 LivingInfoField 获取 history_queries 和 delta_seconds
    - 计算回复的几何论术语密度 geo_density
    - 将 geo_density 传入 update_eta_living 增强自指项
    - 提取关键论断存入向量库
    - 更新 LivingInfoField 而非调用 set_eta_state
    """
    ...
    # 保存对话记录（MySQL 或内存，用于日志/审计，eta 不再依赖它）
    save_conversation(
...
def chat_completions():
    ...
    # v9 改造：从 LivingInfoField 获取 eta，而非 get_eta_state()
    ...
    # v9 改造：从 LivingInfoField 获取 history_queries，而非 compute_history_context
    history_queries = living_field.get_history_queries(session_id)
    delta_seconds = living_field.get_history_delta_seconds(session_id)
...
if __name__ == '__main__':
    ...
    # 注意：MySQL 仍用于对话记录保存，但 eta 不再依赖它
    init_db()
    ...
    logger.info(f"[STARTUP] ===== 几何论AI v9.0 活体信息场版 =====")
    ...
    logger.info(f"[STARTUP] 数据库模式: {'MySQL' if DB_REQUIRED else '内存'} (仅用于对话记录保存，eta 由活体信息场管理)")
