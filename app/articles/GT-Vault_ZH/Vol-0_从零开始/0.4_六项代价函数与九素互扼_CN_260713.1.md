...
- **$\Lambda^2$ 的来源——为什么是 $\Lambda^2$ 而非 $\Lambda$？** 三个扇区分别对应 $S_3$ 的三个共轭类 $C_1, C_2, C_3$。全息屏作为信息承载面，不仅编码各扇区的独立投影强度，还必须编码扇区之间的**信息传递**。信息传递发生在扇区对之间：从扇区 $i$ 到扇区 $j$ 的信号构成一个有序对 $(C_i, C_j)$。三个共轭类之间所有可能的有序对共有 $\Lambda \times \Lambda = 3 \times 3 = 9$ 个。这些是全息屏上不可约的**信息通道**（information channels）——它们彼此独立，因为 $(C_i \to C_j)$ 和 $(C_k \to C_l)$ 在 $(i,j) \neq (k,l)$ 时编码不同的物理过程。

从群论角度可严格验证：群代数 $\mathbb{C}[S_3]$ 在 $S_3 \times S_3$ 的左×右正则作用下，以共轭类为基的转移算符（transition operators）张成一个 $\Lambda \times \Lambda$ 矩阵空间，其独立矩阵元恰为 $\Lambda^2 = 9$。注意这与群代数维数 $|S_3| = 6$ 不矛盾——6 是精细到群元素层面的独立通道数，而 $\Lambda^2 = 9$ 是粗粒化到共轭类（扇区身份）层面的独立通道数。粗粒化后的通道数可以大于群元素数，因为不同群元素可以贡献同一共轭类对的独立关联模式（例如 $S_3$ 中两个不同的 2-循环可以连接同一对共轭类，但各自产生独立的关联相位）。

因此，$90^\circ$ 的全息屏总角度被均匀划分为 $\Lambda^2 = 9$ 个基本通道，每个通道分配 $90^\circ/9 = 10^\circ$。

**$k_0=2$ 的作用——为什么再除以 2？** 极大真正规子群 $A_3 \lhd S_3$ 的指数为 $k_0 = [S_3 : A_3] = 2$。商群 $S_3/A_3 \cong \mathbb{Z}_2$ 作用于全息屏，产生一个定向翻转对称性：每个信息通道 $(C_i \to C_j)$ 与其定向反转 $(C_j \to C_i)$ 在 $\mathbb{Z}_2$ 作用下配对。这 $k_0=2$ 重简并将 $10^\circ$ 的通道单元进一步细化为 $10^\circ/2 = 5^\circ$ 的基本角度量子。
