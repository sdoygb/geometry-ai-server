...
> **语言**：中文 / English  
> **更新规则**：见 §11 维护规则  
> **本次更新**：§5 常数——数值出处更新为 0.1（260710.3），新增 $S_0$、$\Lambda_H$，删除重复条目（零态谱权重、$S_{\min}$），$\hbar_{\text{eff}}$ 中的 ℰ 映射引用替换为后验匹配；§7 角度——新增三角度谱刚性锁定值 $57.93^\circ/26.16^\circ/5.91^\circ$，η 参数更新出处；§10 禁用词——新增 $\mathcal{E}$ 映射（内部映射层）、外部锚点/外部输入参数、物理-几何映射；§11.2 新增 ℰ 映射废弃冲突记录
