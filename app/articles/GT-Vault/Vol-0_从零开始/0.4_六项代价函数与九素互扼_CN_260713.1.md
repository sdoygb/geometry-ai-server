...
**摘要：** 从[#9 三分切丛分解（GT-0.3.0.1）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.1)、[#15 乘积球面几何（GT-0.3.0.3）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.3)和[#10 公理3：全息屏编码条件（GT-0.3.0.6）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.6)出发，构造六项代价函数，建立九素互扼（九个参数通过六组约束相互锁定），求解极值得电子点（$\theta_M^e=57.93^\circ$），并证明Hessian正定性。全部推导自包含，不引用任何外部定理。
...
| 章节 | 输出 | 数学形式 | 引用 |
|:---|:---|---:|:---|
| 0.1 | 谱权重 | $w_M : w_C : w_I = 6 : 2 : 1$ | [#164 谱权重比（GT-0.1.0.9）](./0.1_零维源点与S₃_CN_260713.1.md#GT-0.1.0.9) |
| 0.2 | 互锁常数 | $\Lambda(S_3) = 3$ | [#4 Λ=3（GT-0.2.0.4）](./0.2_谱展开与互锁常数_CN_260713.1.md#GT-0.2.0.4) |
| 0.2 | 紧致常数 | $k_0(S_3) = 2$ | [#4 k₀=2（GT-0.2.0.5）](./0.2_谱展开与互锁常数_CN_260713.1.md#GT-0.2.0.5) |
| 0.2 | 半径比 | $r_M : r_C : r_I = 1 : 1/\sqrt{3} : 1/\sqrt{6}$ | [半径比（GT-0.2.0.6）](./0.2_谱展开与互锁常数_CN_260713.1.md#GT-0.2.0.6) |
| 0.3 | 公理1（三分连续化） | 谱层级→三个连续角度参数 $\theta_M, \theta_C, \theta_I$ | [#9 三分切丛分解（GT-0.3.0.1）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.1) |
| 0.3 | 公理2（乘积球面几何） | $M(a) = S^3 \times S^3 \times S^3$，半径比固定 | [#15 M(a)约束等谱刚性（GT-0.3.0.3）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.3) |
| 0.3 | 公理3（全息屏约束） | $\theta_M + \theta_C + \theta_I = 90^\circ$ | [#10 公理3：全息屏编码条件（GT-0.3.0.6）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.6) |
...
从[#15 乘积球面几何（GT-0.3.0.3）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.3)出发，每个扇区 $i$ 对应一个 $S^3$ 球面，其半径为 $r_i$。球面上的点由角度参数 $\theta_i$ 描述。扇区 $i$ 向全息屏投影的**投影强度**由投影角 $\theta_i$ 的正弦给出：
...
定义域：$\theta_M, \theta_C, \theta_I \in (0^\circ, 90^\circ)$，且 $\theta_M + \theta_C + \theta_I = 90^\circ$（[#10 公理3：全息屏编码条件（GT-0.3.0.6）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.6)）。
...
但并非所有六项函数都等价——系数的选择、每个项的具体函数形式（$\sin$ 而非 $\cos$ 或 $\tan$）都来自[#15 乘积球面几何（GT-0.3.0.3）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.3)和[#10 公理3：全息屏编码条件（GT-0.3.0.6）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.6)。如果公理2中的球面几何改为其他几何（如双曲几何），函数形式就会不同。
...
从0.2章已证明的[#4 Λ=3（GT-0.2.0.4）](./0.2_谱展开与互锁常数_CN_260713.1.md#GT-0.2.0.4)和[#4 k₀=2（GT-0.2.0.5）](./0.2_谱展开与互锁常数_CN_260713.1.md#GT-0.2.0.5)，以及[#10 公理3：全息屏编码条件（GT-0.3.0.6）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.6)（$90^\circ$），我们可以计算全息屏上的基本角度单元：
...
这个推导的每一步都已在前三章中建立：
- $90^\circ$来自[#10 公理3：全息屏编码条件（GT-0.3.0.6）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.6)
- $\Lambda=3$来自[#4 Λ=3（GT-0.2.0.4）](./0.2_谱展开与互锁常数_CN_260713.1.md#GT-0.2.0.4)（S₃的共轭类数）
- $k_0=2$来自[#4 k₀=2（GT-0.2.0.5）](./0.2_谱展开与互锁常数_CN_260713.1.md#GT-0.2.0.5)（S₃的极大真正规子群指数）
...
4. 再由[#10 公理3：全息屏编码条件（GT-0.3.0.6）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.6)得到 $\theta_C^e + \theta_I^e = 90^\circ - 57.93^\circ = 32.07^\circ$
...
- **宏角度 $\theta_i$**：由[#9 三分切丛分解（GT-0.3.0.1）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.1)从谱层级引入，是连续参数
- **扇区曲率 $\kappa_i$**：度量扇区的内禀弯曲程度，由Hessian矩阵的对角元决定
- **渗透函数 $\eta_i$**：描述扇区之间的信息渗透率，由交叉耦合强度决定

### 0.4.8.2 互扼关系（A）：全息屏约束

这是九素互扼中最基本的关系，直接来自[#10 公理3：全息屏编码条件（GT-0.3.0.6）](./0.3_三公理_CN_260713.1.md#GT-0.3.0.6)：
...
从[#164 谱权重比（GT-0.1.0.9）](./0.1_零维源点与S₃_CN_260713.1.md#GT-0.1.0.9)的谱权重6:2:1，我们可以得到扇区曲率之间的比例关系。谱权重反映了扇区的信息容量，而信息容量与曲率相关——曲率越大，信息容量越小（因为曲率大意味着几何结构更紧凑）。
