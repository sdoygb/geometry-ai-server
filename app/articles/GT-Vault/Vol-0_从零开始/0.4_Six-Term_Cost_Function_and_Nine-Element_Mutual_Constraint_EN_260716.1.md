...
```mermaid
flowchart TD
    A["Spectral Weights<br>6:2:1"] --> D["0.4 Six-Term<br>Cost Function"]
    B["Λ=3, k₀=2<br>Radius Ratio"] --> D
    C["Three Axioms:<br>Tripartite Continuation<br>Product Sphere · 90°"] --> D
    D --> E["S(θ) Strictly Convex<br>S_min = 24"]
    D --> F["Nine-Element<br>Mutual Constraint<br>9 params → 2 DOF"]
    D --> G["ΔΘ = 5°"]
    E --> H["S_e = 137.036<br>(via Vol.2 Bridge)"]
    F --> H
    G --> H
```
