...
**⚠️ 定量误差估计**：绝热消除的精度由谱间隙比控制。冻结近似引入的 $\text{tr}(P_{\text{long}})$ 的相对误差为：
...
此误差通过 $\text{Res}_\Sigma \propto \text{tr}(P_{\text{long}})$ 线性传播至 Wodzicki 留数，进而通过 $\chi_L \propto \sqrt{\text{Res}_\Sigma}$ 以平方根传播至长度标度（$\sim 0.33\%$）。**通道数=1 是一个精度约 0.7% 的有效近似，而非严格等于 1 的定理。** 严格处理需要将 7 个法向模式作为有质量激发（massive excitations）纳入谱求和，这会导致 $\text{tr}(P_{\text{long}})$ 的微小修正——目前留待后续工作。
...
**⚠️ KO-维数歧义与标准公式对比**：上述推导依赖一个关键假设——$C_J$ 由总旋量空间 $\mathcal{S}$ 的维数比确定。但在非交换几何标准处理中，Wodzicki 留数的归一化取决于截面算子 $D_\Sigma$ 所在流形的 KO-维数。存在两种计数：

| 计数方式 | KO-维数 | 标准 $C_J$ | 与 $1/2$ 的关系 |
|:---|:---:|:---:|:---|
| 按总流形 $M(a)$ | 9 | $2^{-4}=1/16$ | 8 倍差异 |
| 按截面 $\Sigma=S^2$ | 2 | $2^{-1}=1/2$ | 一致 |

歧义来源：$D_\Sigma$ 作用在 $\Sigma$（2维）但其旋量丛是 $M(a)$（9维）的限制。本文方案等价于按截面 KO-维数 2 计算，与标准公式一致。若未来论证取总流形 KO-维数 9，$C_J \to 1/16$，$\chi_L$ 改变 $\sqrt{8}\approx 2.83$ 倍。**当前将 KO-维数歧义标注为开放问题**，$C_J=1/2$ 的后验数值验证（$\Delta r_{\min}$ vs $\lambda_C$ 偏差 <0.14%）提供支持但不构成严格证明。
