...
**前置依赖：** [2.1 谱三元组构造](./2.1_谱三元组构造.md)、[2.2 长度标度重建](./2.2_长度标度重建.md)
...
代入 $R_\mathcal{M} = a$（物质扇区 $S^3_\mathcal{M}$ 的半径等于 $M(a)$ 的基准半径），$a$ 与 $\chi_L$ 的关系由全息屏面积锁定（§2.2.5 联立求解）：$a^3 = (\chi_L^2/(16\sqrt{3}))^{3/2} / (2\pi^2) \cdot (4\pi)$。经代数化简，所有依赖 $a$、$N_1$、$c$、$\sin^3\theta_M$ 的因子相互抵消，最终得到：
...
该结果不是"诚实话术"或最小输入假设——它是谱互锁条件＋量纲桥自洽性的**必然推论**。任何 $C_m \neq A_\Sigma$ 的选择都会导致谱互锁断裂或量纲桥方程不自洽。
...
与[长度标度 $\chi_L$（GT-2.2.0.4）](./2.2_长度标度重建.md) 和 $\chi_T$ 不同，$K$ 的谱不变量性质使其成为量纲桥中**最稳定的锚点**——它不依赖Wodzicki留数的归一化常数选择（$C_J = 1/2$），也不依赖热核系数的比值（$a_1/a_0$）。$K$ 直接来自Dixmier迹的唯一延拓性质，在谱三元组框架中是最"刚性"的谱数据。
