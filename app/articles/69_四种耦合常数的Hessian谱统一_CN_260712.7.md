...
| 符号 | 数值 | 来源 | 说明 |
|:---:|:---:|:---|:---|
| $\lambda_1^{\text{eff}}$ | 391.05 rad⁻² | 0.6.7定理6.1 | 有效软模（$(\theta_C,\theta_I)$基） |
| $\lambda_2^{\text{eff}}$ | 59324.3 rad⁻² | 0.6.7定理6.1 | 有效硬模（$(\theta_C,\theta_I)$基） |
| $\Lambda_H^{\text{eff}}$ | 151.71 | 0.6.7 | 有效谱比（$(\theta_C,\theta_I)$基） |
| $\Lambda_H$ | 150（裸） | 0.2.1 | 裸谱比（用于$\kappa$和） |

> **⚠️ 坐标基依赖说明（2026-07-15）：** 本文使用的 Hessian 特征值及谱比基于消去 $\theta_M$、保留 $(\theta_C,\theta_I)$ 的约束基（与0.2.1第3.4节一致）。2026-07-15修正的1.4文档（截面Hessian GT-1.4.0.5）使用消去 $\theta_I$、保留 $(\theta_M,\theta_C)$ 的约束基并进一步旋转至 $(\xi,\eta)$ 解耦坐标，得到特征值 $(1.94\times10^2, 1.18\times10^5)$、刚度比 $\approx607$。两套数值描述的是同一约束曲面上的同一双线性型，但在不同坐标基下矩阵表示不同——特征值通过合同变换（$P^T H P$）而非相似变换关联，数值差异是预期的。物理预言对基的选择敏感：本文使用 $(\theta_C,\theta_I)$ 基做出的预言（$\alpha_s$, $\sin^2\theta_W$, $G_{9D}$）若改用 $(\xi,\eta)$ 基需重新计算。此基依赖性是框架的诚实标注项，非矛盾。参见1.6文档§6.2.4的对应说明。
