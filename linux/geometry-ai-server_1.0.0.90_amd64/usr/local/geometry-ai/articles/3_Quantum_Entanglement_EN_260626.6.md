...
This article reconstructs the theory of quantum entanglement on the basis of the three axioms of Geometric Theory, the Ninefold Interlocking Constraint, and all theorems of the 0.X series (version 260626.6), within the framework of holographic screen pixel structure, using the **single core mapping** (ℰ: ℳ–𝒞 waist-edge coupling → electromagnetic interaction) as the sole physical mapping input. Principal results: (i) Within the framework of holographic screen pixel structure, the information field equation is rigorously derived from the local harmonic oscillator theorem of 0.2.1 (Theorem 3.5.10); pixels are defined by ℐ-sector harmonic oscillator eigenfunctions, with minimum resolvable angle $\Delta\xi_{\min}=2\lambda_2^{-1/4}$; (ii) A geometric definition of entanglement is established—two $S$-minima on the geometric constraint section $\Sigma_\eta$ achieve geometric maximal entanglement through ℐ-sector freezing and ℳ-sector antipodal locking, with variational stability jointly guaranteed by the Ninefold Interlocking Constraint and cross-sector coupling structure; (iii) The Two-Level Theorem is proved—the low-energy non-trivial excitation space of a geometrically maximally entangled pair is exactly 2-dimensional; (iv) Under the physical mapping conventions, the correlation function $C(\vec{a},\vec{b})=-\vec{a}\cdot\vec{b}$ is derived; (v) In the hypothetical limit $\lambda_2^{\text{eff}}\to\infty$, information field localization is proved, giving a CHSH combination algebraic upper bound of $2$; (vi) Under the condition that $\lambda_2^{\text{eff}}$ is finite and the correlation mapping is accepted, the supremum of the CHSH combination is proved to be $2\sqrt{2}$. In the Appendix, the mapping capacity of Geometric Theory for interference phenomena is demonstrated under the external input hypothesis of a double-slit topological defect; this Appendix does not satisfy the 0.0.X + single core mapping closed-loop criterion.


# 1. Holographic Screen Pixel Theorem

## 1.0 Declaration of the Ninefold Interlocking Constraint

The following structures treated in this chapter are subject to the Ninefold Interlocking constraint (0.3.1 Theorem 3.1a; 0.2.1 §1.4):
...
- **Bott periodicity cutoff** (Corollary 1.1a): the cutoff $n_{\text{eff}}\le 7$ is the pixel-level manifestation of $N_{\text{eff}}=7$ in the Ninefold Interlocking (0.0.7 Theorem 6.3);
...
**(i) Ninefold Interlocking Constraint** (0.3.1 Theorem 3.1a): the first-order condition $[[D,a],Jb^*J^{-1}]=0$ imposes hard-mode freezing in the ℐ-sector; the high stiffness of $\lambda_2^{\text{eff}}$ strongly suppresses deviations in the ℐ-direction, so that $\xi_A=\xi_B$ is a stable condition;

**(ii) Cross-sector coupling structure** (0.1 Theorems 4.2/4.3): the percolation matrix $\Phi$ and cross-sector coupling $H^W=[[2w,w'-2w],[w'-2w,2w]]$ (waist edge $w=-0.7885$, base edge $w'=-2.444$) constitute the algebraic constraints of the ℳ-𝒞-ℐ trifurcated tangent bundle;
...
By the completeness axiom $\theta_M+\theta_C+\theta_I=\pi/2$ and the 2-dimensional constraint of $\Sigma_\eta$, the effective projection of the ℳ-sector onto $\Sigma_\eta$ is realized by the projection map $P_{\mathcal{M}}:\mathcal{M}\to\Sigma_\eta$ of 0.0.6. Under the Ninefold Interlocking constraint (0.3.1 Theorem 3.1a), the first-order condition of the tri-decomposition frame $\{e_1,\dots,e_9\}$ gives the tangent space of the ℳ-sector ($e_1,e_2,e_3$) a projection of rank 2 onto the 2-dimensional tangent plane of $\Sigma_\eta$. Hence the 4-fold degeneracy is partially broken, retaining only 2 zonal modes adapted to the tangent space of $\Sigma_\eta$, denoted $\Phi_+,\Phi_-$, satisfying $\Phi_-(\vec{x})=\Phi_+(-\vec{x})$ (antipodal antiphase).
...
(7) **Closed-Loop Status**: Chapters 1–6 of this article are completed within the closure of 0.0.X + single core mapping, introducing no independent hypotheses beyond the Ninefold Interlocking and the three axioms. Convention A is guaranteed by the geometric configuration of Definition 2.2; Convention B is an external correspondence of the physical mapping layer, labeled as an open core problem. Appendix A demonstrates the mapping capacity of Geometric Theory for interference phenomena under the external input hypothesis of a double-slit topological defect, and does not satisfy the closed-loop criterion.

(8) **Version Upgrade**: 260622.6 → 260626.6. Principal changes: corrected information field equation (constant potential → harmonic oscillator potential); comprehensive parameter upgrade to effective metric values ($\lambda_1^{\text{eff}}=391.05$, $\lambda_2^{\text{eff}}=59324.3$); explicit introduction of Ninefold Interlocking constraint (§1.0); strict labeling of all theorem/proposition statuses; corrected source citations; Chapter 7 moved to Appendix A.
...
> - **§1.0 (new)**: Explicit introduction of the Ninefold Interlocking constraint declaration, connecting ℐ-sector freezing, Bott periodicity cutoff, soft-hard mode separation, and tri-decomposition frame structure to the Ninefold Interlocking framework of 0.3.1 §3.1a and 0.2.1 §1.4
...
> - **§2.1**: Single core mapping declaration aligned to 0.1 §1.3; variational stability argument of Definition 2.2 changed from vague "0.2.1 soft mode theorem" to explicit three sources: (i) Ninefold Interlocking constraint (ii) cross-sector coupling structure (iii) weak coupling condition
...
> - **§3.1**: Rank-2 projection argument of Two-Level Theorem connected to Ninefold Interlocking first-order condition (0.3.1 Theorem 3.1a), replacing previously vague "0.0.7 constraint mapping" citation; ℓ=0 freezing subtraction supplemented with compatibility note regarding completeness axiom
...
> - **Source citation corrections**: Removed all non-existent "0.2.1 Theorem 3.5.4 (soft mode theorem) → phase connection" citations; 0.0.7 constraint mapping citation changed to 0.0.6 projection mapping + Ninefold Interlocking first-order condition; 0.2.1 Theorem 3.5.10 citation method corrected
