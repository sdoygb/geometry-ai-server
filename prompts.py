...
- view_article：当向量检索提供的片段不够详细、需要阅读完整文章时，调用 view_article 获取全文。文件名从【参考资料】中的文章标签获取。limit 建议设 3000-5000（每次读取足够多的内容，避免反复调用浪费轮数）。offset 用于跳到文章指定位置。大多数情况下向量检索片段已足够，无需调用。注意：工具调用最多8轮，请高效使用。
- personal_write：重要信息可以写入个人数据库。
- 参考资料已通过向量语义检索自动注入下方【参考资料】区域，无需手动查找。
